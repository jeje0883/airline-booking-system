...
export default function Payment() {
  ...
  useEffect(() => {
    if (bookedData) {
      console.log('Saving bookedData to localStorage');
  ...
  // 1. Simulate payment request
  const redirectToPaymentLink = async () => {
    console.log(`Redirecting to payment link for: ${paymentMethod}`);
    if (paymentMethod) {
      // Simulate redirect to external payment provider (e.g., Credit Card, PayPal, etc.)
      return Promise.resolve({ status: 200, success: true }); // Placeholder for actual redirection
    }
  ...
  const savePayment = async () => {
    ...
    const response = await fetch(`${process.env.REACT_APP_API_URL}/payments`, {
      ...
      headers: {
        'Content-Type': 'application/json',
        // Use guest email if user is not logged in
        Authorization: `Bearer ${localStorage.getItem('token') || guestEmail}`,
      },
      body: JSON.stringify(paymentPayload),
    });
  ...
  // 4. Update booking with payment ID
  const updateBookingWithPaymentId = async (paymentId) => {
    console.log(`Updating booking ID: ${bookingId} with payment ID: ${paymentId}`);

    const response = await fetch(
      `${process.env.REACT_APP_API_URL}/bookings/${bookingId}/updatepayment`,
      {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
    ...
    const updatedBooking = await response.json();
    console.log('Booking update response:', updatedBooking);
  ...
  const updateFlightWithBooking = async () => {
    ...
    console.log(
      'Updating flight with new booking and available seats:',
      bookingId,
      seatClass,
      noPassenger
    );
    ...
    const response = await fetch(
      `${process.env.REACT_APP_API_URL}/commercialflights/${bookedData.commercialFlightId._id}/addbooking`,
      {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
    ...
    const updatedFlight = await response.json();
    console.log('Flight update response:', updatedFlight);
  ...
  const handlePaymentRequest = async () => {
    try {
      ...
      const paymentData = await savePayment(); // Save payment details
      console.log('Payment saved, data:', paymentData);

      await updateBookingWithPaymentId(paymentData._id); // Update booking with payment ID
      console.log('Booking updated with payment ID');

      await updateFlightWithBooking(); // Update the flight with booking ID and seats
      console.log('Flight updated with booking');
    ...
    <div>
      {!showTicket ? (
        <div className="container">
          ...
          <div>
            <p>
              By clicking the ‘Continue’ button below, I confirm that I have read, understood, and
              accept all the conditions set by the airline.
            </p>
          </div>
      ...
      <Modal show={isProcessing} backdrop="static" keyboard={false} centered>
          <Modal.Body className="d-flex align-items-center justify-content-center text-center">
            ...
            <p className="mb-0">Processing payment, please wait ...</p>
          </Modal.Body>
      </Modal>
    </div>
  );
