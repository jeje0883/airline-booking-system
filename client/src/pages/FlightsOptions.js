...
export default function FlightOptions() {
  ...
  let isContinueDisabled = true; // Default to true

  if (selectedFlight) {
    const selectedClass = selectedClasses[selectedFlight._id] || 'economySeat';
    const availableSeats = selectedFlight.availableSeats?.[selectedClass] || 0;
    isContinueDisabled = availableSeats === 0;
  }
  ...
  return (
    <div className="flight-options">
      <div className="container mt-5">
        ...
        <div className="button-container m-4 d-flex justify-content-between">
          <BackButton link="/flights" />
          <ContinueButton onClick={handleContinue} disabled={isContinueDisabled} />
        </div>
      </div>
    </div>
  );
